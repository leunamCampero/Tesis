...
Sea $G$ y $F$ un campo. Llamaremos a $\mathbb{F}$ una
$F$-representacióna de $G$ si $\mathbb{F}$ es un homomorfismo de $G$
...
el espacio subyacente de $\mathbb{F}$). Además la dimensión de $V$
será el grado de $\mathbb{F}$.

Análogo a lo visto en el cápitulo \ref{cha:Representaciones de
  grupos}, una $F$-representación será llamada fiel, si es un
isomorfismo. Dos $F$-representaciones $\mathbb{F}_{1}$,
$\mathbb{F}_{2}$ de $G$ son similares si tienen el mismo espacio
vectorial subyacente $V$ y si existe una transformación lineal no
singular $S$ de $V$ tal que
$\mathbb{F}_{1}(g)=S^{-1} \mathbb{F}_{2}(g)S$ para todo $g \in G$.
...
Si $\mathbb{F}$ es una $F$-representación de $G$ con $V$ el espacio vectorial subyacente, entonces $\mathbb{F}$ puede ser extendido al homomorfismo de anillos de $F(G)$ en el anillo de transformaciones lineales de $V$ definiendo $\mathbb{F}  \left\{ \sum_{g \in G} a(g)g \right\} = \sum_{g \in G} a(g) \mathbb{F}(g) $. De ésta manera $V$ tiene un $F(G)$-módulo impuesta, y el $F(G)$-módulo contiene un neutro multiplicativo. Reciprocamente, un $F(G)$-módulo finito generado que también contiene un nuetro multiplicativo da origen a una únicamente determinada $F$-representación de $G$. Así el estudio de $F$-representaciones de $G$ es equivalente a estudiar $F(G)$-módulos finitos generados.
\backmatter
