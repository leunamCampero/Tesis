...
Sea $G$ y $F$ un campo. Llamaremos a $\digamma$ una
$F$-representacióna de $G$ si $\digamma$ es un homomorfismo de $G$
...
el espacio subyacente de $\digamma$). Además la dimensión de $V$
será el grado de $\digamma$.

Análogo a lo visto en el cápitulo \ref{cha:Representaciones de
  grupos}, una $F$-representación será llamada fiel, si es un
isomorfismo. Dos $F$-representaciones $\digamma_{1}$,
$\digamma_{2}$ de $G$ son similares si tienen el mismo espacio
vectorial subyacente $V$ y si existe una transformación lineal no
singular $S$ de $V$ tal que
$\digamma_{1}(g)=S^{-1} \digamma_{2}(g)S$ para todo $g \in G$.
...
Si $\digamma$ es una $F$-representación de $G$ con $V$ el espacio
vectorial subyacente, entonces $\digamma$ puede ser extendido al
homomorfismo de anillos de $F(G)$ en el anillo de transformaciones
lineales de $V$ definiendo
$\digamma \left\{ \sum_{g \in G} a(g)g \right\} = \sum_{g \in G}
a(g) \digamma(g) $. De ésta manera $V$ tiene un $F(G)$-módulo
...
Sea $F$ cualquier campo. Si $\digamma_{1}$ y $\digamma_{2}$ son $F$-representaciones de $G$ con espacio vectorial subyacente $V$ y $W$ respectivamente. Entonces $\digamma_{1} \oplus \digamma_{2}$,  $\digamma_{1} \otimes \digamma_{2}$ son $F$-representaciones de $G$ con espacios vectoriales subyacentes $V \oplus W$, $V \otimes W$ respectivamente.
