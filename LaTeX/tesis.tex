...
$\widetilde H_{k}(M_n)$.
 ...
 $G$-módulos se escriben \textbf{encerrados en cuadros}, ya que en
...
A(a)=\chi(a)$. Entonces~$\chi(a)$ es una función que toma valores en
...
\begin{example}
  ...
  $T = \left \{ (1), (14), (24), (34) \right \}$.  Por ejemplo
...
ecuación se mantiene cuando nos referimos a $\sigma$ y $\sigma^{'}$
...
\end{algorithm}  
  ...
  \end{equation*}
\begin{algorithm}[H]
\caption{Calcular la descomposición en irreducibles de $\widetilde H_{1}(M_5)$ como $S_n$-módulo.}
\begin{algorithmic}
\STATE $n = 5, r = 1$
\STATE bo = Bouc$_{-}$theorem$(r,n)$
\PRINT bo
...
\begin{equation}
  \widetilde H_{0}(M_{7})\cong\widetilde H_{0}(K(M_{7}))\cong_{S_{7}} \mathbb{C}
\end{equation}
\begin{equation}
...
$\widetilde H_{7}(K(M_{8}))$, ya que la memoria de la computadora fue
...
\end{example}
donde $h_0 = e_0 = 1$.
La \textbf{La característica de Frobenius} $\textup{ch}(V)$ de una
...
$\tau \in S_m$, cuya multiplicación está dada por: Se tiene lo
siguiente:
\begin{equation*}
